...
Based on the plots created with the code above, the model with all of the predictors had the highest RMSE, which means that it did not "fit" the data set as well as the other models. The model with only the main effect had the second lowest RMSE values, and the model with the interctions "fit" the dataset the best. However, the RMSE values are still very high across the three models, so these models are not the most accurate in prediction. 
